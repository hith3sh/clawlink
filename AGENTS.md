...
- **2026-05-20 — LinkedIn `LINKEDIN_GET_MY_INFO` 429 APPLICATION DAY limit (shared-app throttle) + generic long-lived-rate-limit classifier improvement.** User reported `LINKEDIN_GET_MY_INFO` failing with `429 "Resource level throttle APPLICATION DAY limit for calls to this resource is reached."` against their LinkedIn connection on 2026-05-19 21:22 UTC. Verified by calling the same tool through ClawLink's hosted API against the user's own connection 40 on 2026-05-20 → returned the user's identity cleanly (`id="5YeT9ma4Vr"`, name="Hithesh"). LinkedIn's daily quota resets at UTC midnight, confirming the diagnosis: **APPLICATION DAY limit is LinkedIn throttling the OAuth APPLICATION, not the user**. On Composio Managed mode (`ac_cbilq2TPjvjU`) every ClawLink user shares one daily quota against Composio's LinkedIn app, so a single noisy session can DoS every other user's LinkedIn posting flow (since `LINKEDIN_CREATE_LINKED_IN_POST` requires the URN from this tool first — there's no alternative URN source on this connection). Compounding factors: (a) Composio's connector falls back from `/v2/me` (403 — deprecated `r_liteprofile` scope, see 2026-05-09 LinkedIn audit entry) to `/v2/userinfo`, so each call burns from two endpoints' pools; (b) Composio's connector auto-retries 3× on 429 with backoff, multiplying the strain; (c) ClawLink's classifier was marking 429s as `retryable: true` unconditionally, so the LLM would loop and burn further calls. Resolution (shipped, deploys next): (1) Strengthened `LINKEDIN_GET_MY_INFO` override in `config/composio-tool-overrides.mjs` to instruct the LLM to call ONCE per session and reuse the cached `id` — names the APPLICATION DAY failure mode and the shared-quota reality explicitly. (2) Added `isLongLivedRateLimit(message)` predicate in `src/lib/composio/backend-client.ts` matching `\bAPPLICATION DAY|DAY limit|daily.{0,30}(quota|limit|cap|exceeded)|per[\s-]?day\b`. When `isComposioRateLimit` AND `isLongLivedRateLimit` both match, the error gets `code: "rate_limit_day"` instead of `"rate_limit"`. (3) Updated `classifyIntegrationError` in `worker/integrations/base.ts:267` to set `retryable: error.code !== "rate_limit_day"` — APPLICATION DAY limits now classify as non-retryable so the LLM stops looping. Unit-tested the regex against 6 cases (LinkedIn APPLICATION DAY, Google per-day, Twitter daily, burst-rate, userRateLimitExceeded, generic too-many) — all classify correctly. Strategic followup: LinkedIn is a strong BYO migration candidate — our own LinkedIn app would have its own daily quota isolated from other Composio users, and the LinkedIn audit entry from 2026-05-09 already notes BYO is non-viable for organization/admin scopes but IS viable for the basic `openid`/`profile`/`w_member_social` set that `LINKEDIN_GET_MY_INFO` and `LINKEDIN_CREATE_LINKED_IN_POST` need. Worth considering once a customer complains repeatedly.

- **2026-05-20 — Reddit `LIST_SUBREDDIT_POST_FLAIRS` is mod-only + uncovered a Composio status-classifier bug that affected ALL providers.** User reported `REDDIT_LIST_SUBREDDIT_POST_FLAIRS` 403ing on `r/SideProject`. After the user connected their own Reddit account, re-tested against `ca` connection id 328 via the hosted `POST /api/tools/reddit_list_subreddit_post_flairs/execute` route: **identical 403 reproduced** with `code: "provider_unavailable", retryable: true`, while a contrasting call to `reddit_get_scopes` on the same connection returned the full scope catalog cleanly. Confirms two things: (a) Reddit's `/r/<sub>/api/link_flair_v2` endpoint is moderator-only — non-mods get 403 regardless of OAuth scopes, no scope unlocks it; (b) ClawLink's classifier was misreporting the failure as retryable. Composio's own per-tool description already calls (a) out ("Reddit may return empty or deny access if the authenticated user cannot set link flair and is not a moderator"). For (b): traced to `src/lib/composio/backend-client.ts:executeComposioToolRequestOnce`, which **hardcoded `status=502` for every `successful: false` response from Composio's `/tools/execute`**, regardless of what the upstream provider (Reddit/Facebook/Instagram/etc.) actually returned. That falsely-502 then triggered `isComposioTransient` (matches `status >= 500`), got `kind: "transient", code: "provider_unavailable"`, which made `classifyIntegrationError` set `retryable: true`. **Every 4xx from any Composio-backed provider was being mis-classified as retryable for the entire history of this code path.** Resolution (shipped, deploys next): (1) added `REDDIT_LIST_SUBREDDIT_POST_FLAIRS` + `REDDIT_CREATE_REDDIT_POST` overrides in `config/composio-tool-overrides.mjs` — the flair-list override states the mod-only nature explicitly and prescribes the fallback ("ask the user for the desired flair text/ID"); the create-post override defines the right flair-handling sequence (post without flair first → only call list-flairs if a flair-required error fires → if list 403s, ask the user). (2) Added `extractUpstreamStatus(payload)` helper in `backend-client.ts` that reads `mercury_last_http_status_code` (Composio's top-level field) then falls back to `data.status_code`, then to 502 only if neither is present. `executeComposioToolRequestOnce` now passes the real upstream status to `toComposioRequestError` instead of hardcoded 502. After deploy, Reddit's 403 + "Forbidden" message will route through `isComposioMissingScope` → `type: "missing_scopes", retryable: false` (semantically loose since Reddit's gate is role-based not scope-based, but behaviorally correct: non-retryable + clear LLM signal). 5xx from upstream still flows through `isComposioTransient` and stays retryable. Typechecked clean (`tsc --noEmit` no errors on backend-client.ts). Lesson: not every recurring 4xx warrants a code fix on the *override* side — sometimes it's the *classifier* under it that's wrong, and one bug at that layer can silently distort error semantics across every provider.

- **2026-05-20 — Instagram `image_url` file-share-host trap (corrected after live testing).** User reported three consecutive `INSTAGRAM_POST_IG_USER_MEDIA` calls for the same SaathiMed launch post; first two failed (`temp.sh/.../banner.png`, `temp.sh/.../banner.jpg`), third succeeded (`http://184.174.37.88/saathimed/banner.jpg` — self-hosted). I initially assumed two gotchas (file-share host AND JPEG-only-for-feed) based on Composio's docs language and training-data assumptions, but **live tested against the user's own `ca_SM93Ve-hGUwg` IG connection with container-creation-only (no publish)** and found the JPEG-only assumption was wrong. Test matrix: (1) `upload.wikimedia.org/.../Yellow_Lab_puppy.JPG` → FAIL `"The media could not be fetched from this URI"` — Wikimedia blocks `facebookexternalhit`; (2) `upload.wikimedia.org/.../HD_transparent_picture.png` → SUCCESS container created — PNG works fine for IG feed; (3) `picsum.photos/seed/clawlink/1080.jpg` → SUCCESS; (4) `placehold.co/1080x1080.jpg` → SUCCESS; (5) the user's original `temp.sh/.../banner.png` → FAIL with the exact error `"The image format is not supported... the URL returned an error page instead of an image"` — which confirms temp.sh returns an HTML download page to Meta's fetcher, not the bytes. So the real trap is **host behavior, not format**. Resolution (shipped, deploys next): added `INSTAGRAM_FEED_IMAGE_URL_FIELD_DESCRIPTION`, `INSTAGRAM_FEED_IMAGE_URL_VALIDATOR`, and `INSTAGRAM_DM_IMAGE_URL_VALIDATOR` in `config/composio-tool-overrides.mjs`. Both validators deny file-share/temp hosts (`temp.sh`, `transfer.sh`, `tempfile.io`/`tempfile.sh`, `file.io`, `anonfiles.com`, `wetransfer.com`/`we.tl`, `mega.nz`, `sendspace.com`, `mediafire.com`) plus non-direct cloud share URLs (`drive.google.com/file/`/`open`/`uc`, `dropbox.com/...?dl=0`). NO format-extension denial — PNG/WebP/etc. are not categorically rejected by IG. Wired into `INSTAGRAM_POST_IG_USER_MEDIA` (feed) and `INSTAGRAM_SEND_IMAGE` (DM). Descriptions explain the HTML-download-page failure mode and suggest CDN re-hosting + name working hosts (picsum.photos, placehold.co, S3/R2/Cloudinary). Validator takes effect at next deploy via the live `src/lib/server/arg-guards.ts` boundary-guard — LLM gets a `type: "validation"` error with hint on the first bad call. Lesson for next time: Composio's per-tool description hints at constraints but don't trust them as hard rules — live-test container creation (non-destructive) against the user's own connection before encoding format-level denials. Containers expire in 24h and don't post, so this is a safe verification path.

- **2026-05-20 — Facebook placeholder-`page_id` trap + reproducible Composio Managed appsecret_proof outage.** User reported two failures: (a) `FACEBOOK_GET_PAGE_DETAILS` with `page_id: "me"` rejected by Composio's Pydantic validator: `"Invalid page_id format: 'me'. Facebook Page IDs must be numeric strings"`; (b) `FACEBOOK_LIST_MANAGED_PAGES` failing with `400 "API calls from the server require an appsecret_proof argument"` on `graph.facebook.com/v20.0/me/accounts`. **Issue (b) is a reproducible upstream bug, not transient.** Two test rounds 5 minutes apart against the same active Facebook connections on `ac_rrK7_9ggxQP7` (Composio Managed — confirmed via dashboard screenshot showing the "Composio Managed (Recommended)" credentials option with 11 scopes: public_profile, email, pages_show_list, pages_read_engagement, pages_manage_posts, pages_manage_engagement, pages_read_user_content, pages_manage_metadata, pages_messaging, read_insights, business_management): the first round had 10/11 succeed (caught a brief healthy window — false confidence), the second round had 5/5 sampled connections fail identically with `appsecret_proof`. Root cause is on Composio's side: their Meta app either has "Require App Secret Proof for Server API Calls" enabled in Advanced Settings, or their connector stopped sending `appsecret_proof = HMAC-SHA256(access_token, app_secret)`. Either way, **zero ClawLink-side fix exists in Managed mode** — we don't own the Meta app, the app_secret, or the connector code. Trace IDs captured: `AD_mJ36gA1Pk6Sk10nm-nkE`, `AfEYbvSGW_UXqQPxLrJ7j2S`. Unblock paths, in order: (1) report to Composio support with trace IDs and ask them to fix their managed Meta app (fastest); (2) migrate Facebook to BYO OAuth — register our own Meta app with `appsecret_proof` OFF, request the 11 scopes (most need Meta app review), create a new Composio auth config with "Your Own Credentials" + our client_id/client_secret, point `COMPOSIO_AUTH_CONFIG_MAP['facebook']` at the new id; (3) flip Facebook tools to `coming_soon` until upstream is fixed. **For (a)** — same pattern as the LinkedIn `urn:li:person:self` trap: LLM substituting `me` for the User-node shortcut, which Facebook only honors on the User node, never on Pages. Resolution (shipped, deploys next): added shared `FACEBOOK_PAGE_ID_FIELD_DESCRIPTION` + `FACEBOOK_PAGE_ID_VALIDATOR` constants to `config/composio-tool-overrides.mjs` (`denyPatterns` rejects `me`/`self`/`current_user`/`my_page`/`<…>`/`{…}`/`placeholder|your_page_id|insert_id|replace_id`; `allowPatterns` requires `/^\d+$/`). Wired into `FACEBOOK_GET_PAGE_DETAILS`, `FACEBOOK_GET_PAGE_POSTS`, `FACEBOOK_CREATE_POST`, `FACEBOOK_GET_PAGE_CONVERSATIONS`, `FACEBOOK_GET_PAGE_INSIGHTS`. Strengthened the `descriptionPrefix` on `FACEBOOK_GET_PAGE_DETAILS` specifically to name the `me` failure mode. Because the boundary-guard in `src/lib/server/arg-guards.ts` is already live (unlike when the LinkedIn fix was first written), `fieldValidators` takes effect at the next deploy — LLM gets a `type: "validation"` error with an actionable hint, never reaches Composio with the bad arg. (a)'s fix is independent of (b) — ship it now so the trap is gone the moment Composio fixes the upstream block.
